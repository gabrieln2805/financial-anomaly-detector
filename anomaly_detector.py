...
def _build_context(row, df: pd.DataFrame, ticker: str) -> str:
    ...
    fired = []
    if row.get("flag_return"):
        fired.append(f"price return {ret_pct:+.2f}%")
    if row.get("flag_volume"):
        fired.append(f"volume {vol_mult:.1f}x avg")
    if row.get("flag_vol"):
        fired.append(f"realized vol {real_vol:.1f}%")
